...
class AutoRefreshTokenMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        if not access_token and not refresh_token:
            return await handle_no_tokens(request, call_next)

        if not access_token and refresh_token:
            return await handle_only_refresh_token(request, call_next, refresh_token)

        if access_token and not refresh_token:
            return await handle_only_access_token(request, call_next)

        return await call_next(request)


async def handle_no_tokens(request: Request, call_next):
    logger.info("No access or refresh token found, proceeding without authentication")
    return await call_next(request)


async def handle_only_refresh_token(request: Request, call_next, refresh_token: str):
...
async def handle_only_access_token(request: Request, call_next):
